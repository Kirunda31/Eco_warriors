...
export default async function Home() {
  const [programs, projects, metrics, featuredStory, partners, heroProjects, events, news] = await Promise.all([
    ...
    prisma.news.findMany({ where: { status: 'published' }, orderBy: { createdAt: 'desc' }, take: 2 }),
  ]);
  ...
  return (
    <div className="overflow-hidden">
      ...
      {(events.length > 0 || news.length > 0) && (
        <section className="mx-auto max-w-6xl px-6 py-16 sm:py-24"><div className="flex flex-wrap items-end justify-between gap-4"><div><p className="text-sm font-bold uppercase tracking-[0.16em] text-emerald-700">Stay connected</p><h2 className="mt-2 text-3xl font-bold tracking-tight">News & events</h2></div><div className="flex gap-4"><Link href="/news" className="font-semibold text-emerald-800 hover:underline">All news →</Link><Link href="/events" className="font-semibold text-emerald-800 hover:underline">All events →</Link></div></div><div className="mt-8 grid gap-6 lg:grid-cols-2"><div><div className="mb-4 flex items-center justify-between"><h3 className="text-lg font-bold">Latest news</h3><Link href="/news" className="text-sm font-semibold text-emerald-800 hover:underline">View news →</Link></div><div className="grid gap-5">{news.map(article => <Link key={article.id} href="/news" className="group overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg">{article.featuredImage ? <img src={article.featuredImage} alt={article.title} className="h-52 w-full object-cover transition duration-500 group-hover:scale-105" /> : <div className="flex h-52 items-end bg-gradient-to-br from-teal-800 to-emerald-600 p-5 text-sm font-bold uppercase tracking-wide text-white">Eco Warriors news</div>}<div className="p-5"><p className="text-xs font-bold uppercase tracking-wide text-emerald-700">{article.category}</p><h4 className="mt-2 text-xl font-bold text-slate-900">{article.title}</h4><p className="mt-2 line-clamp-3 text-sm leading-6 text-slate-600">{article.content}</p><span className="mt-4 inline-block text-sm font-semibold text-emerald-800">Read article →</span></div></Link>)}{!news.length && <p className="rounded-xl border border-dashed p-6 text-sm text-slate-500">News will be shared here soon.</p>}</div></div><div><div className="mb-4 flex items-center justify-between"><h3 className="text-lg font-bold">Upcoming events</h3><Link href="/events" className="text-sm font-semibold text-emerald-800 hover:underline">View events →</Link></div><div className="grid gap-5">{events.map(event => <Link key={event.id} href="/events" className="group overflow-hidden rounded-2xl border border-emerald-100 bg-white shadow-sm transition hover:-translate-y-1 hover:shadow-lg">{event.featuredImage ? <img src={event.featuredImage} alt={event.title} className="h-52 w-full object-cover transition duration-500 group-hover:scale-105" /> : <div className="flex h-52 items-end bg-gradient-to-br from-amber-500 to-emerald-700 p-5 text-sm font-bold uppercase tracking-wide text-white">Eco Warriors event</div>}<div className="p-5"><p className="text-xs font-bold uppercase tracking-wide text-emerald-700">{event.date.toLocaleDateString('en-UG', { day: 'numeric', month: 'short', year: 'numeric' })}</p><h4 className="mt-2 text-xl font-bold text-slate-900">{event.title}</h4><p className="mt-1 text-sm text-slate-500">{event.time} · {event.location}</p><p className="mt-3 line-clamp-2 text-sm leading-6 text-slate-600">{event.description}</p><span className="mt-4 inline-block text-sm font-semibold text-emerald-800">View event →</span></div></Link>)}{!events.length && <p className="rounded-xl border border-dashed p-6 text-sm text-slate-500">No upcoming events right now.</p>}</div></div></div></section>
      )}
